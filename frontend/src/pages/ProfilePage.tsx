...
import { useConceptMachineVisibility, useSetConceptMachineVisibility } from '@/hooks/useConceptMachineVisibility';
...
export function ProfilePage() {
  ...
  const { isVisible: conceptMachineVisible, isLoading: visibilityLoading } = useConceptMachineVisibility();
  const { mutate: setConceptMachineVisible, isPending: isSettingVisibility } = useSetConceptMachineVisibility();
  ...
  const handleToggleConceptMachineVisibility = (checked: boolean) => {
    setConceptMachineVisible(checked, {
      onSuccess: () => {
        toast.success(checked ? 'ConceptMachine page is now visible' : 'ConceptMachine page is now hidden');
      },
      onError: (error) => {
            ...
            <CardHeader>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription>Manage administrator access and page visibility</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <Label htmlFor="conceptmachine-visibility">ConceptMachine pagina weergeven</Label>
                    <p className="text-xs text-muted-foreground">
                      Schakel de ConceptMachine pagina in of uit voor alle gebruikers
                    </p>
                  </div>
                  <Switch
